class Public::UsersController < ApplicationController
  ...
  # 日付を指定して学習時間の合計と学習項目を取得
    ...
    weekly_learning_hours = @user.posts.where(created_at: start_date..end_date).group("strftime('%Y-%m-%d', created_at)").sum(:learning_hour)
    
    @weekly_learning_hours = []
    (start_date..Date.today).reverse_each do |date|
    formatted_date = date.strftime("%Y-%m-%d")
    logger.debug "Weekly Learning Hours: #{weekly_learning_hours.inspect}"
